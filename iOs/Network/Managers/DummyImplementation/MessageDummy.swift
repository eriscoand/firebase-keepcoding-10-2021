...
public class MessageDummy: MessageManager {
    ...
    public func list(onSuccess: @escaping ([Message]) -> Void, onError: ErrorClosure?) {
        ...
        let d1 = MessageKind.text(value1)
        let m1 = Message.init(sender: john, kind: d1, type: MessageTypeEnum.text.rawValue, value: value1)
        
        let value2 = "Now I have a machine gun! HO! HO! HO!"
        let d2 = MessageKind.text(value2)
        let m2 = Message.init(sender: hans, kind: d2, type: MessageTypeEnum.text.rawValue, value: value2)
        
        let value3 = "diehard.jpg"
        let mediaItem = ImageMediaItem.init(image: UIImage(named: "diehard.jpg")!)
        let d3 = MessageKind.photo(mediaItem)
        let m3 = Message.init(sender: john, kind: d3, type: MessageTypeEnum.image.rawValue, value: value3)
